...
# --- THE HYBRID PARAMETER ---
# How many "Phantom Points" is a Win worth?
WIN_BONUS = 10.0 
...
def solve_single_state(state_int):
    ...
    if p1_p >= 3 or p2_p >= 3 or (MiniQwixxEnv.is_row_locked(p1_r, p2_r) and MiniQwixxEnv.is_row_locked(p1_b, p2_b)):
        ...
        # ---> THE HYBRID TERMINAL EVALUATION <---
        score_diff = s1 - s2
        if score_diff > 0:
            hybrid_val = score_diff + WIN_BONUS
        elif score_diff < 0:
            hybrid_val = score_diff - WIN_BONUS
        else:
            hybrid_val = 0.0 # Tie
                ...
                for w2_idx, a_w2 in enumerate(WHITE_ACTIONS):
                    ...
                    for a_c in COLOR_ACTIONS:
                        ...
                        if is_term:
                            ...
                            diff = s1 - s2
                            if diff > 0: val = diff + WIN_BONUS
                            elif diff < 0: val = diff - WIN_BONUS
                            else: val = 0.0
                        else:
...
def run_hybrid_induction():
    ...
    cores = mp.cpu_count()
    print(f"Firing up {cores} CPU cores for HYBRID MODEL Tracking (Bonus = {WIN_BONUS})...")
    ...
    p1_hybrid_margin = final_V_hybrid[0, 0]
    
    print(f"\nBackward Induction Complete in {round((end_time - start_time)/60, 2)} minutes!")
    print(f"Start State Expected Hybrid Advantage: {p1_hybrid_margin:.4f} (Combines expected points + win probability bonus)")

if __name__ == '__main__':
    run_hybrid_induction()