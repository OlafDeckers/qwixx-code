...
def solve_single_state(state_int):
    ...
    if p1_p >= 3 or p2_p >= 3 or (MiniQwixxEnv.is_row_locked(p1_r, p2_r) and MiniQwixxEnv.is_row_locked(p1_b, p2_b)):
        ...
        # ---> THE CORE WIN PROBABILITY CHANGE <---
        if s1 > s2: val = 1.0     # P1 Wins
        elif s1 < s2: val = -1.0  # P2 Wins
        else: val = 0.0           # Tie
            
        shared_V_win[state_int, 0] = val
        shared_V_win[state_int, 1] = val
        return
    ...
    for active_player in [1, 2]:
                ...
                for w2_idx, a_w2 in enumerate(WHITE_ACTIONS):
                    ...
                    for a_c in COLOR_ACTIONS:
                        ...
                        if is_term:
                            ...
                            val = shared_V_win[next_s, next_idx]
        ...
        shared_V_win[state_int, active_idx] = expected_win_margin

def run_win_prob_induction():
    ...
    cores = mp.cpu_count()
    print(f"Firing up {cores} CPU cores for WIN PROBABILITY Tracking...")
    ...
    # Calculate absolute win percentage for Player 1 from the margin
    p1_margin = final_V_win[0, 0]
    p1_win_pct = ((p1_margin + 1) / 2) * 100 
    
    print(f"\nBackward Induction Complete in {round((end_time - start_time)/60, 2)} minutes!")
    print(f"Start State Margin: {p1_margin:.4f}")
    print(f"Expected Player 1 Win Rate (if both play perfectly): {p1_win_pct:.2f}%")

if __name__ == '__main__':
    run_win_prob_induction()