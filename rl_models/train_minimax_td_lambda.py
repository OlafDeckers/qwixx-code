...
def worker_process(args):
    ...
    for episode in range(1, episodes + 1):
        ...
        # Reset trace memory at the start of every game
        eligibility_traces = {}
        ...
        while True:
            ...
            # --- TD-LAMBDA LOGIC (Using shared memory) ---
            ...
            for (trace_state, trace_player) in list(eligibility_traces.keys()):
                shared_V_learned[trace_state, trace_player] += alpha * delta * eligibility_traces[(trace_state, trace_player)]
                eligibility_traces[(trace_state, trace_player)] *= lambda_decay
        ...
        if worker_id == 0 and local_checkpoint_mark > 0 and episode % local_checkpoint_mark == 0:
            ...
            chkpt_name = f'data/checkpoints/V_rl_td_lambda_{global_milestone}M.npy'
...
def train_multicore(total_episodes=20000000):
    ...
    print(f"Firing up {cores} CPU cores for TD-Lambda! Each runs {episodes_per_core} episodes.")
    ...
    np.save('data/V_rl_minimax_td_lambda.npy', final_V_learned)
    np.save('data/mse_history_td_lambda.npy', np.array(final_mse_history))
    
    end_time = time.time()
    print(f"\nMassive 20M Episode TD-Lambda Complete in {round((end_time - start_time)/60, 2)} minutes!")
