...
def train_minimax_q(episodes=1000000, alpha_start=0.1, alpha_end=0.01, epsilon_start=1.0, epsilon_end=0.01):
    ...
    print(f"Beginning MINIMAX Q-Learning for {episodes} episodes...")
    ...
    for episode in range(episodes):
        ...
        while True:
            ...
            for w1_idx, a_w1 in enumerate(white_actions):
                for w2_idx, a_w2 in enumerate(white_actions):
                    ...
                    for a_c in color_actions:
                        ...
                        step_reward = get_score_diff(next_state) - current_score_diff
                        
                        if is_terminal: future_val = step_reward
                        else: future_val = step_reward + V_learned[next_state, next_active_idx]
            ...
            # --- ADVERSARIAL RL TARGET (GAME THEORY) ---
            if random.random() < epsilon:
                v_target = random.choice(payoff_matrix.flatten())
            else:
                # Instead of max(), the agent calculates the true Nash Equilibrium of its expected values
                if active_player == 1:
    ...
    np.save('data/V_rl_minimax.npy', V_learned)
    np.save('data/mse_history_minimax.npy', np.array(mse_history))
    
    end_time = time.time()
    print(f"\nMinimax Q-Learning Complete in {round((end_time - start_time)/60, 2)} minutes!")
