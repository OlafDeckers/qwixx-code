...
def train_dense_rewards(episodes=1000000, alpha_start=0.1, alpha_end=0.01, epsilon_start=1.0, epsilon_end=0.01):
    print("Loading exact Nash values and DAG...")
    V_exact = np.load('data/V_nash.npy')
    dag = np.load('data/topological_dag.npy')
    
    # Extract the true Nash difference from the 3D array
    ...
    print(f"Beginning DENSE REWARD Q-Learning for {episodes} episodes...")
    ...
    np.save('data/V_rl_dense.npy', V_learned)
    np.save('data/mse_history_dense.npy', np.array(mse_history))
    
    end_time = time.time()
    print(f"\nDense Q-Learning Complete in {round((end_time - start_time)/60, 2)} minutes!")
